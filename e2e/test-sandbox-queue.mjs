...
try {
  ...
  handle.queueFollowUp("third", "message", "third-id");
  first.resolve();
  ...
  assert.deepEqual(calls, [
    { prompt: "first", mode: "message" },
    { prompt: "second", mode: "goal" },
    { prompt: "third", mode: "message" },
  ...
  assert.throws(() => bounded.queueFollowUp("overflow", "message", "overflow"), /At most 5/);
  ...
  const failed = await retrying.sendPrompt("retry me", "message", "attempt-1");
  assert.equal(failed.status, "failed");
  assert.equal((await retrying.sendPrompt("retry me", "message", "attempt-1")).status, "failed");
