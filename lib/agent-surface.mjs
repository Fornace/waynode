...
export class AgentSurface {
  ...
  annotateSubmissions(entries) {
    ...
    for (const entry of entries) {
      ...
      const match = active.find((record) => {
        ...
        return entry.text === record.prompt || entry.text === wrapped
          || entry.text === record.wirePrompt || entry.text.includes(record.prompt);
      });
  ...
  getSubmission(submissionId) {
    const live = this.submissions.publicRecord(this.submissions.get(submissionId));
    if (live) return live;
    const persisted = SubmissionLedger.lookup(submissionId);
    return persisted?.sessionId === this.sessionId ? persisted.submission : null;
  }
