...
import { secretCodec } from "./secret-crypto.mjs";

const encrypt = (plaintext) => secretCodec().encrypt(plaintext);
const decrypt = (payload) => secretCodec().decrypt(payload);
...
/** Seed a global encrypted secret once; later starts never overwrite rotation. */
export function ensureGlobalSecret(keyName, value) {
  const existing = db.prepare(
    "SELECT id FROM secrets WHERE scope = 'global' AND key_name = ? LIMIT 1"
  ).get(keyName);
  if (existing) return false;
  setSecret({ scope: "global", keyName, value });
  return true;
}

/** Look up a secret's scope/space so authorization can be enforced per-id. */
export function getSecret(id) {
  return db.prepare("SELECT id, scope, org_id, space_id, key_name FROM secrets WHERE id = ?").get(id);
}
...
export function getSecretValue({ scope, spaceId, orgId, keyName }) {
  const row = scope === "global"
    ? db.prepare("SELECT encrypted_value FROM secrets WHERE scope = 'global' AND key_name = ?").get(keyName)
    : scope === "org"
...
export function getSecretsEnv(spaceId) {
  const space = spaceId
    ? db.prepare("SELECT org_id FROM spaces WHERE id = ?").get(spaceId)
    : null;
  const rows = [
    ...db.prepare("SELECT key_name, encrypted_value FROM secrets WHERE scope = 'global'").all(),
    ...(space?.org_id
      ? db.prepare("SELECT key_name, encrypted_value FROM secrets WHERE scope = 'org' AND org_id = ?").all(space.org_id)
      : []),
    ...(spaceId
