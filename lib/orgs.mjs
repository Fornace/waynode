...
export function acceptInvite(token, userId) {
  ...
  if (!existing) {
    // Seat limits only apply on explicit Waynode Cloud deployments. Enforce at
    // acceptance rather than invite creation so stale/unused links can never
    // reserve or oversubscribe a paid organization.
    if (billingEnabled) {
      const quota = checkQuota(invite.org_id);
      const seats = (PLANS[quota.plan] || PLANS.free).seats;
      const members = db.prepare("SELECT COUNT(*) AS count FROM org_members WHERE org_id = ?").get(invite.org_id).count;
      if (!['active', 'trialing'].includes(quota.status) || members >= seats) return { error: "seat_limit" };
    }
    db.prepare("INSERT INTO org_members (org_id, user_id, role) VALUES (?, ?, ?)").run(invite.org_id, userId, invite.role);
  }
