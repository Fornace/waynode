...
export class SandboxedAgentHandle {
  ...
  sendPrompt(prompt, isGoal = false, submissionId) {
    ...
    const { record } = this.submissions.create({ id: submissionId, prompt, isGoal, status: "starting" });
    this.followUps.push(record);
    void this._drain();
    return record.completion;
  }

  queueFollowUp(prompt, isGoal = false, submissionId) {
    const existing = submissionId && this.submissions.get(submissionId);
    if (existing) return existing.completion;
    if (!this.streaming) return this.sendPrompt(prompt, isGoal, submissionId);
    ...
    const { record } = this.submissions.create({ id: submissionId, prompt, isGoal, status: "queued" });
    this.followUps.push(record);
    return record.completion;
  }
  ...
  async _drain() {
    ...
    while (this.followUps.length) {
      ...
      try {
        await this._runPrompt(turn.prompt, turn.isGoal);
        this.submissions.settle(turn, "completed");
      } catch (error) {
        this.submissions.settle(turn, "failed", error);
      }
    }
    this.currentSubmission = null;
    this.streaming = false;
  ...
  async _runPrompt(prompt, isGoal) {
    ...
    try {
      const result = await this.runMessage({ session: this.session, prompt, isGoal, onChunk });
  ...
  async abort() {
    return { cancelled: false, reason: "This hosted run cannot be interrupted safely yet" };
  }
