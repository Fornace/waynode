...
export function seedPiComponents({
  ...
  const changed = [];
  const sourceExtensions = join(sourceDir, "extensions");
  if (existsSync(sourceExtensions)) {
    const destinationExtensions = join(agentDir, "extensions");
    const expectedDigest = directoryDigest(sourceExtensions);
    if (directoryDigest(destinationExtensions) !== expectedDigest) {
      replacePackage(sourceExtensions, destinationExtensions, expectedDigest);
      changed.push("waynode-extensions");
    }
  }
