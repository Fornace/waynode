...
export class AgentHandle {
  ...
  async sendPrompt(prompt, mode = "message", submissionId, { resumed = false } = {}) {
    ...
    try {
      await this._send({
        type: "prompt",
        // A resumed turn is a continuation: the original prompt (and its
        // goal wrap, if any) is already in pi's context via --continue.
        message: resumed ? prompt : goalPrompt(prompt, record.mode),
