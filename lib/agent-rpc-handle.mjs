...
export class AgentHandle {
  ...
  _onAgentEnd() {
    ...
    // Clear unconditionally: a stale abort id must never cancel-label a
    // later queued turn that completed naturally.
    this.abortRequestedId = null;
  ...
  async sendPrompt(prompt, mode = "message", submissionId) {
    ...
    try {
      // streamingBehavior makes a busy agent queue the prompt instead of
      // rejecting with "Agent is already processing". Our `streaming` flag is
      // bookkeeping for status/routing; pi's own state is authoritative, and
      // the two can race (HTTP send vs agent_end/agent_start between queued
      // follow-ups). With followUp, that race queues instead of failing.
      await this._send({ type: "prompt", message: goalPrompt(prompt, record.mode), streamingBehavior: "followUp" });
    } catch (error) {
      this.streaming = false;
      this.currentSubmission = null;
  ...
  queueFollowUp(prompt, mode = "message", submissionId) {
    ...
    this._send({ type: "follow_up", message: goalPrompt(prompt, record.mode) }).catch((error) => {
        ...
        // If agent_end already promoted this record to currentSubmission
        // (streaming=true) and pi then rejected the follow_up, no agent_start
        // will ever come — clear the promotion or the handle wedges busy
        // forever (reaper skips streaming handles, /message 409s).
        if (this.currentSubmission === record) {
          this.currentSubmission = null;
          this.streaming = false;
        }
        this.submissions.settle(record, "failed", error);
