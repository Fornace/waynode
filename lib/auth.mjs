...
if (config.gitlab.clientId) {
  passport.use(new GitLabStrategy({
    ...
    try {
      if (req?.user) {
        // Linking GitLab to existing session
        db.prepare(`UPDATE users SET gitlab_id = ?, gitlab_token = ? WHERE id = ?`)
          .run(profile.id, accessToken, req.user.id);
        return done(null, { id: req.user.id });
      }
