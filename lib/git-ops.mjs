...
function run(cwd, args, { maxBuffer = 20 * 1024 * 1024 } = {}) {
  const r = spawnSync("git", ["-C", cwd, "-c", "core.quotepath=false", NO_LOCK, ...args], {
    encoding: "utf8",
    maxBuffer,
  });
...
export function commitSelected(cwd, { files, summary, description }) {
 ...
 */
export function pull(cwd, { mode = "ff-only" } = {}) {
  return withLock(cwd, () => {
    try {
      if (mode === "merge") {
        const out = run(cwd, ["pull", "--no-rebase", "--no-edit"]);
        return { mode, output: out };
      }
      if (mode === "rebase") {
        const out = run(cwd, ["pull", "--rebase"]);
      ...
      const out = run(cwd, ["pull", "--ff-only"]);
 ...
 * On conflict, aborts the merge and reports the conflicted files so the repo
 * is never left in a half-merged state the sidebar can't escape. Returns
 * { merged } on success or { aborted, conflicts } on conflict.
 */
export function mergeBranch(cwd, { branchName }) {
  ...
  return withLock(cwd, () => {
    try {
      const out = run(cwd, ["merge", target, "-m", `Merge ${target}`]);
