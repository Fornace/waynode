...
function run(cwd, args, { maxBuffer = 20 * 1024 * 1024, configs = [] } = {}) {
...
export function pull(cwd, { mode = "ff-only", identity } = {}) {
  ...
  return withLock(cwd, () => {
    try {
      if (mode === "merge") {
        const out = run(cwd, ["pull", "--no-rebase", "--no-edit"], { configs: cfg });
        return { mode, output: out };
      }
      if (mode === "rebase") {
        const out = run(cwd, ["pull", "--rebase"], { configs: cfg });
      ...
      const out = run(cwd, ["pull", "--ff-only"], { configs: cfg });
