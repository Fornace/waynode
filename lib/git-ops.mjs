 ...
 *  - Self-host keeps the soft `piBusy` warning. Hosted execution blocks host
 *    Git while a sandbox is active so guest config cannot race the scrub.
 */
import { spawnSync } from "child_process";
import { existsSync } from "fs";
import { gitConfigArgs } from "./git-identity.mjs";
import { trustedGitEnvForCwd } from "./git-creds.mjs";

const NO_LOCK = "--no-optional-locks";

/**
 * Thrown when a space's local_path no longer exists on disk (directory
 * deleted outside the app, or a stale/orphaned space row). Routes should
 * catch this via `e.spaceDirMissing` and return a clean 404/409 instead of
 * letting git's raw `fatal: cannot change to '...'` stderr propagate as an
 * unhandled 500.
 */
class SpaceDirMissingError extends Error {
  constructor(cwd) {
    super("Space directory not found on disk — it may have been deleted outside the app");
    this.name = "SpaceDirMissingError";
    this.spaceDirMissing = true;
    this.cwd = cwd;
  }
}
...
function run(cwd, args, {
  maxBuffer = 20 * 1024 * 1024, configs = [], auth = false, literalPathspecs = false,
} = {}) {
  if (!existsSync(cwd)) throw new SpaceDirMissingError(cwd);
  const confArgs = ["-c", "core.quotepath=false"];
  for (const c of configs) { confArgs.push("-c", c); }
  // Hosted worktrees are guest-writable. Re-scrub local config immediately
  // before every trusted host git process, including read-only status/diff.
  const env = trustedGitEnvForCwd(cwd, { auth });
  const globalArgs = literalPathspecs ? ["--literal-pathspecs"] : [];
  const r = spawnSync("git", [...globalArgs, "-C", cwd, ...confArgs, NO_LOCK, ...args], {
    encoding: "utf8",
    maxBuffer,
    env,
  });
...
// Destructive helpers live in a separate module to keep this core file small,
// but share the exact same hardened runner and per-worktree write queue.
export { run as runGit, withLock as withGitLock };
...
export function getStatus(cwd) {
  ...
  for (const line of out.split("\n")) {
    ...
    if (line.startsWith("# branch.head ")) currentBranch = line.slice("# branch.head ".length);
      ...
      const originalPath = tabIdx >= 0 ? pathField.slice(tabIdx + 1) : null;
