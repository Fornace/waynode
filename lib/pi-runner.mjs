...
import { mkdirSync, readFileSync, existsSync, readdirSync, realpathSync } from "fs";
...
export function embeddedPiResourceArgs(agentDir = join(homedir(), ".pi", "agent")) {
  ...
  if (existsSync(waynodeJournal)) args.push("--extension", waynodeJournal);
...
const GOAL_PREFIX = `You must use the create_goal tool with token_budget 50000000 to create a goal for the following task. Use that full 50,000,000-token budget; do not choose a smaller budget. Then work autonomously until you can call update_goal with status "complete". Task: `;

export function getPiArgs({ session, prompt, mode, sessionDir = session.pi_session_dir, agentDir }) {
  const model = resolvePiModel(session).spec;
  const submissionMode = normalizeSubmissionMode(mode);
...
export function getSandboxPiArgs({ session, prompt, mode, workspaceDir }) {
  const sessionDir = getSandboxSessionDir({ workspaceDir, sessionDir: session.pi_session_dir });
  return getPiArgs({ session, prompt, mode, sessionDir, agentDir: "/root/.pi/agent" });
}

export async function runPiMessage({ session, prompt, mode, onChunk = null, onSandbox = null }) {
  const space = db.prepare("SELECT local_path, org_id FROM spaces WHERE id = ?").get(session.space_id);
  if (!space) throw new Error("Space not found");

  // Session rows can be created while a background clone is still running.
  // Keep the session directory lazy so it cannot make `git clone` reject its
  // destination; create it only once the agent is actually starting.
  ...
  try {
    ...
    if (sandboxAvailable) {
      const llmEnv = await sandboxChatLlmEnv(session, config, { orgId: space.org_id });
      ...
      try {
        return await runInSandbox({
          ...
          spaceId: session.space_id, onChunk, onSandbox,
        });
      } catch (err) {
        if (!canFallbackToDirect()) throw err;
        try { onSandbox?.(null); } catch {}
...
export async function runInSandbox({ args, cwd, env, spaceId, onChunk = null, onSandbox = null }) {
  ...
  const sandbox = await Sandbox.builder(name)
    ...
    .replace()
    .create();
  // Expose the live VM to the caller so an abort/session-delete can stop it.
  try { onSandbox?.(sandbox); } catch {}
...
export async function runPiTerminal({ session, cols = 80, rows = 24 }) {
  ...
  if (sandboxAvailable) {
    const llmEnv = await sandboxChatLlmEnv(session);
    try {
...
export { readGoalStatus, computeSessionTokenTotal } from "./pi-session-reader.mjs";
