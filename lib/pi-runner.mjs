...
export async function runPiMessage({ session, prompt, isGoal = false }) {
  ...
  if (await isSandboxAvailable()) {
    try {
      return await runInSandbox({ args, cwd: space.local_path, env, spaceId: session.space_id });
    } catch (err) {
...
async function runInSandbox({ args, cwd, env, spaceId }) {
