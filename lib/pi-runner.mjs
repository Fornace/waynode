...
export function getPiArgs({ session, prompt, isGoal, sessionDir = session.pi_session_dir }) {
...
export function getSandboxPiArgs({ session, prompt, isGoal, workspaceDir }) {
  const sessionDir = getSandboxSessionDir({ workspaceDir, sessionDir: session.pi_session_dir });
  return getPiArgs({ session, prompt, isGoal, sessionDir });
}
...
export async function runPiMessage({ session, prompt, isGoal = false, onChunk = null }) {
  ...
  const directArgs = getPiArgs({ session, prompt, isGoal });
  ...
  try {
    ...
    if (sandboxAvailable) {
      ...
      const sandboxArgs = getSandboxPiArgs({ session, prompt, isGoal, workspaceDir: space.local_path });
      try {
