...
export class SubmissionLedger {
  ...
  /** Boot-time sweep: every still-active row belonged to a dead process. */
  ...
  /** Rows a restart left unfinished, oldest first (active + interrupted). */
  static recoverableRows(sessionId) {
    try {
      return db
        .prepare("SELECT id, prompt, mode, status FROM submissions WHERE session_id = ? AND status IN ('queued','starting','running','interrupted') ORDER BY datetime(created_at)")
        .all(sessionId);
    } catch {
      return [];
    }
