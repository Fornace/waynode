...
export function normalizeSubmissionMode(modeOrGoal = "message") {
  if (modeOrGoal === true) return "goal";
  if (modeOrGoal === false || modeOrGoal == null) return "message";
  if (["message", "goal", "hammersmith"].includes(modeOrGoal)) return modeOrGoal;
  throw new TypeError("Unknown submission mode");
}

export function goalPrompt(prompt, modeOrGoal) {
  return normalizeSubmissionMode(modeOrGoal) === "goal"
...
export class SubmissionLedger {
  ...
  create({ id = createRequestId(), prompt, mode, isGoal = false, status }) {
    const existing = this.records.get(id);
    if (existing) return { record: existing, duplicate: true };
    let resolve;
    let reject;
    const completion = new Promise((done, fail) => { resolve = done; reject = fail; });
    const normalizedMode = normalizeSubmissionMode(mode ?? isGoal);
    const record = { id, prompt, mode: normalizedMode, isGoal: normalizedMode === "goal", status, completion, resolve, reject };
  ...
  publicRecord(record) {
    ...
    const { id, prompt, mode, isGoal, status, error } = record;
    return { id, prompt, mode, isGoal, status, ...(error ? { error: error.message || String(error) } : {}) };
  }
