...
import { repairInterruptedToolCalls } from "./pi-tool-recovery.mjs";
...
export async function resumeSession(session, row, { reason = "server-restart" } = {}) {
  ...
  let handle;
  try {
    const repair = repairInterruptedToolCalls(session.pi_session_dir);
    if (repair.repaired > 0) {
      console.log(
        `[recovery] finalized ${repair.repaired} interrupted tool call(s) for ${session.id} `
        + `(${repair.restored} restored, ${repair.uncertain} uncertain)`,
      );
    }
