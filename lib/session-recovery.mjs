...
/**
 * Boot-time scan: flip still-active rows to interrupted, then proactively
 * resume them (bounded concurrency so a restart storm does not fork N agents
 * at once). Runs exactly once per server process: rows created by this live
 * process are current work, not stale recovery candidates.
 */
export async function resumeInterruptedSessions({ concurrency = 2, log = console.log } = {}) {
  if (bootScanDone) return { resumed: 0, failed: 0 };
  bootScanDone = true;
  const interruptedCount = SubmissionLedger.markAllInterrupted();
