...
export { structuredRunSnapshot, hostedOuterTimeoutMs, writeRuntimeConfig };

// Hammersmith's built-in pi engine passes the prompt as "@{spec}", but pi
// documents `pi [options] [@files...]`: a leading @word is a file mention, so
// every hosted run died in ~2s with `Error: File not found: <dir>/TASK` before a
// single model turn — misfiled by the harness as the model ignoring instructions.
// Declaring the engine here (this file is ours) is that built-in minus the stale
// "@". The spec must stay one argv element: hammersmith does not shell-quote it,
// so its own `bash -c "… -p {spec}"` init template exits 127 on a multi-line spec.
function writeRuntimeConfig(path, stateDir) {
  writeFileSync(path, [
    ...
    "[engines.pi]",
    'bin = "pi"',
    'args_template = ["-p", "{spec}", "--no-session"]',
    "sandbox_args = []",
    "full_access_args = []",
...
export class HostedMicrosandboxHammersmithRunner extends HammersmithRunner {
  ...
  async execute({ job, session, stateDir }) {
    ...
    try {
      ...
      const workerEnv = await hammersmithWorkerLlmEnv(session);
