...
export async function getAgent(session) {
  ...
  if (!handle || handle.dead) repairInterruptedToolCalls(session.pi_session_dir);
...
export function activeStreamingCount() {
  return listActiveAgents().filter((agent) => agent.streaming).length;
}
