...
export const PLANS = {
  ...
  hammersmith: { name: "Hammersmith", price: 8.99, storageBytes: 2 * 1024 ** 3, tokensPerMonth: 50_000_000, seats: 1 },
};
...
export function checkQuota(orgId, now = Date.now()) {
  ...
  return {
    ...
    storage: {
      ...
      // Strict `>`: a free plan has storageBytes 0, and `0 >= 0` reported
      // every free-plan org as permanently over-quota. "At the limit" is not
      // "over the limit" — matches storage-quota.mjs's own strict admission gate.
