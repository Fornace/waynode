...
try {
  ...
  db.exec(`CREATE UNIQUE INDEX IF NOT EXISTS hammersmith_submission_identity
    ON hammersmith_jobs(submission_id) WHERE submission_id IS NOT NULL;
...
function sameSubmission(job, { ownerId, sessionId, spaceId, jobDescription }) {
  return job.owner_id === ownerId && job.session_id === sessionId
    && job.space_id === spaceId && job.job_description === jobDescription;
}
...
export function createHammersmithJob({
  ...
  const create = () => transaction(() => {
    ...
    if (existing) {
      if (!sameSubmission(existing, { ownerId, sessionId, spaceId, jobDescription })) throw ownershipConflict();
      return existing;
    }
  ...
  try {
    ...
    if (String(error.message).includes("UNIQUE constraint failed")) {
      const duplicate = getHammersmithJobBySubmission(submissionId);
      if (duplicate) {
        if (!sameSubmission(duplicate, { ownerId, sessionId, spaceId, jobDescription })) throw ownershipConflict();
        return duplicate;
      }
...
export function getHammersmithJobBySubmission(submissionId) {
  return db.prepare("SELECT * FROM hammersmith_jobs WHERE submission_id = ?").get(submissionId) || null;
}
