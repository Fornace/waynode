...
import { credsForOwner } from "./git-creds.mjs";
...
export function cloneRepo(repoUrl, branch = "main", userId, orgId, auth = {}) {
  ...
  // Embed the owner's provider token for HTTPS clone (GitHub: x-access-token,
  // GitLab: oauth2). SSH urls are left untouched (authed by key, not token).
  let cloneUrl = repoUrl;
  if (auth.authUser && auth.authToken) {
    cloneUrl = repoUrl.replace(/^https:\/\//, `https://${auth.authUser}:${auth.authToken}@`);
  } else {
    const creds = credsForOwner(repoUrl, userId);
    if (creds && /^https:\/\//.test(repoUrl)) {
      cloneUrl = repoUrl.replace(/^https:\/\//, `https://${creds.user}:${creds.token}@`);
    }
  }
...
function resolveCloneUrl(repoUrl, userId, auth = {}) {
  ...
  const creds = credsForOwner(repoUrl, userId);
  if (creds && /^https:\/\//.test(repoUrl)) {
    return repoUrl.replace(/^https:\/\//, `https://${creds.user}:${creds.token}@`);
  }
  return repoUrl;
}
