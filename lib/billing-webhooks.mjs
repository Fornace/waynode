...
import { revokeOrgLlmKey } from "./org-llm-key.mjs";
...
function applySubscriptionDeletion(event, subscription, orgId) {
  ...
  // Promptly disable the org's gateway key. Correctness doesn't depend on
  // this (the grant snapshot check re-validates on every turn); this just
  // closes the window between cancellation and the next turn attempt.
  void revokeOrgLlmKey(orgId);
}
