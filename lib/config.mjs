...
export const config = {
  ...
  // Terminal tab in sandboxed mode: the interactive pi TUI needs a long-lived
  // bidirectional pty, which the microsandbox SDK doesn't expose cleanly. So
  // when sandboxed mode is active, the terminal is disabled by default (it
  // would otherwise run untrusted code on the HOST as root — the very thing
  // sandboxing prevents). Set WAYNODE_ALLOW_HOST_TERMINAL=1 to override on a
  // trusted/single-tenant host.
  allowHostTerminal: process.env.WAYNODE_ALLOW_HOST_TERMINAL === "1",
  ...
  // Billing (hosted waynode.fornace.net only). Entirely optional: every field
  // defaults to empty/undefined, and lib/billing.mjs no-ops (or throws a
  // clear "billing not configured" error on write paths) whenever
  // stripe.secretKey is unset. Self-host images ship with none of this set,
  // so the Billing UI stays hidden and nothing here is ever required to boot.
  //
...
// True only when this deployment is configured to run hosted billing.
// Self-host installs leave STRIPE_SECRET_KEY unset, so this is false and the
// billing routes/UI stay fully inert.
export const billingEnabled = !!config.stripe.secretKey;
