...
export const config = {
  ...
  llm: {
    ...
    // Mantice ADMIN token, used server-side only (org-llm-key.mjs) to mint
    // and disable per-org gateway keys. Never enters a sandbox env. When
    // unset, hosted chat falls back to the shared sandboxRuntimeKey.
  ...
  // SIGTERM drain window: how long a deploy waits for streaming agent turns
  // to settle before exiting (recovery continuation resumes the rest).
  drainMs: Number(process.env.WAYNODE_DRAIN_MS ?? 90_000),
  ...
  //   STRIPE_PRICE_HAMMERSMITH
  //                          — price_... id for the per-org hosted Hammersmith
  //                            add-on ($8.99/mo). Optional: it stays OUT of
  //                            priceIds so an unset value never flips
  //                            billingEnabled off; it only gates the add-on.
  stripe: {
    ...
    hammersmithPriceId: process.env.STRIPE_PRICE_HAMMERSMITH || "",
  },
};
