...
// Server-owned acceptance check for a mutating run. The npm gates are also
// conditioned on node_modules: a hosted Space is cloned without dependencies
// and the sandbox denies egress to the npm registry, so demanding `npm test`
// there failed every run in seconds on missing modules instead of judging the
// work. Verify what is actually verifiable — the spirit of --if-present.
const MUTATING_CHECK = "git diff --check && if [ -f package.json ] && [ -d node_modules ]; then npm test --if-present && npm run build --if-present; fi";
  ...
  build({ jobId, jobDescription, workdir, taskKey = HOSTED_TASK_KEY, engine = "pi", mutating = true }) {
    ...
    return {
      ...
      tasks: [{
        ...
        expect_files: [],
        verified: mutating
          ? "The server-owned repository check found no whitespace errors and, when package.json and installed dependencies are both present, the declared npm tests and build passed."
