...
- [x] **Fixed — this was a real, more serious finding than expected**: confirmed the ROOT `Dockerfile` (not just `sandbox/Dockerfile`) baked the live fornace-llm API key into an image layer at build time, with none of the sandbox image's network-scoping justification — extractable via `docker history`/`docker save` by anyone with the image. Removed the bake-in; added `lib/pi-config.mjs` to write the pi provider config from `LLM_BASE_URL`/`LLM_API_KEY` env vars at container **startup** instead (mirrors `ensureGitAskpass()`'s existing pattern), wired into `server.js`. `sandbox/Dockerfile`'s bake-in is untouched and remains the only correct use of that pattern (verified via grep — it's now the only Dockerfile with the key).
